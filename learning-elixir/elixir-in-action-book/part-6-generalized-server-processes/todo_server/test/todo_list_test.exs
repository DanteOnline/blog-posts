defmodule TodoListTest do
  ...
  test "todo server test" do
    ...
    TodoServer.add_entry(todo_server, %{date: @some_date, title: @some_title})

    result_entries = [%{id: 1, date: @some_date, title: @some_title}]
    assert TodoServer.entries(todo_server, @some_date) == result_entries

    TodoServer.update_entry(todo_server, 1, &Map.put(&1, :title, "new title"))

    result_entries = [%{id: 1, date: @some_date, title: "new title"}]
    assert TodoServer.entries(todo_server, @some_date) == result_entries

    TodoServer.delete_entry(todo_server, 1)
    assert TodoServer.entries(todo_server, @some_date) == []
